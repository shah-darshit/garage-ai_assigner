...
def complete_and_archive_task(job_id, outcome_score):
    print(f"\n--- Completing and Archiving Task for Job ID: {job_id} ---")
    conn = None
    try:
        ...
        if not task_data:
            return False, f"No 'Assigned' task found with Job ID {job_id}."
        
        assigned_engineer_id = task_data['Engineer_Id']
        ...
        history_record = {
            'Job_ID': job_id, # Create a unique history Job ID
        ...
        print(f"Task of Job {job_id} successfully archived to job_history.")


        cursor.execute("DELETE FROM job_card WHERE job_card_id = ?", (job_id,))
        conn.commit()
        print(f"Task {job_id} removed from live job_card table.")

        return True, f"Task {job_id} successfully completed and archived."
